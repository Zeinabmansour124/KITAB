...
class exchange {
    ...
    public function count_active($userId) {
        $stmt = $this->db->prepare("SELECT COUNT(*) as count FROM exchanges WHERE (user_requesting_id = :uid OR user_offering_id = :uid2) AND status IN ('pending', 'accepted', 'in_progress')");
        $stmt->execute(['uid' => $userId, 'uid2' => $userId]);
        $result = $stmt->fetch(PDO::FETCH_OBJ);
        return $result ? (int)$result->count : 0;
    }
    ...
    public function recuperer_completed($userId) {
    ...
    public function recuperer_pending($userId) {
        $stmt = $this->db->prepare("SELECT * FROM exchanges WHERE user_offering_id = :uid AND status = 'pending' ORDER BY created_at DESC");
